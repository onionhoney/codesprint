	...
	for (int i = 0; i < t; i++) {
		...
		while (!finished) {
			...
			for (auto& entry : wormholes) {
				for (auto & innerEntry : wormholes) {
					if (innerEntry.first == entry.first)
						continue;
					if (dp[entry.first] + wormholes[entry.first] < dp[innerEntry.first]) {
						finished = false;
						dp[innerEntry.first] = dp[entry.first] + wormholes[entry.first];
					}
					if (dp[innerEntry.first] + wormholes[innerEntry.first] < dp[entry.first]) {
						finished = false;
						dp[entry.first] = dp[innerEntry.first] + wormholes[innerEntry.first];
					}
				}
			}
		}
		...
		// 	for (auto& entry : wormholes) {
		// 		if (dp[entry.first] == INT_MAX || entry.first == j)
		// 			continue;
