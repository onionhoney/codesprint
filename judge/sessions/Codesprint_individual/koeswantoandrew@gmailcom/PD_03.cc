...
int main() {
	...
	for (int i = 0; i < instructions.size(); i++) {
		begin = instructions[i].m_begin;
		end = instructions[i].m_end;

		for (int j = 0; j < i; j++) {
			testBegin = instructions[j].m_begin;
			testEnd = instructions[j].m_end;
			
			if (testBegin >= begin && testBegin <= end) {
				testBegin = end + 1;
				instructions[j].m_begin = end+1;
			}
			if (testEnd <= end && testEnd >= begin) {
				testEnd = begin - 1;
			...
			else if (instructions[j].m_begin < begin && instructions[j].m_end > end) {
			...
			{
				instructions[j].m_begin = testBegin;
				instructions[j].m_end = testEnd;
			}
		}
	}
